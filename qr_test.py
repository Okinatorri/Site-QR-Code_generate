...
BG_IMAGE_DEFAULT = os.path.join(OUTPUT_DIR, "elka_u_kamina_1.jpg")  
...
def qr_genirating(url, foreground_color="#000000", background_color=None,
    ...
    for f in glob.glob(os.path.join(OUTPUT_DIR, "qr_*.png")):
        if os.path.getmtime(f) < time.time() - 30:  
            os.remove(f)
    ...
    if shadow_enabled:
        ...
        dark_overlay = Image.new("RGBA", qr_image.size, shadow_color + '80')  
